...
export const LoaderFive = ({ text }) => {
  ...
  const parts = isRTL ? handleArabicSplitting(text.split(" ")) : text.split("");

  function handleArabicSplitting(sentence) {
    // looping on every word and adding space except last word
    let newSentence = []
    ...
    // adding last word
    newSentence.push(sentence.at(-1))
  ...
  return (
    <div
      className={`${isRTL ? "font-cairo" : "font-sans"} font-bold [--shadow-color:var(--color-neutral-500)] dark:[--shadow-color:var(--color-neutral-100)]`}
    >
      {parts.map((char, i) => (
        <motion.span
          key={i}
